...
async def get_transactions(
    ...
    try:
        transactions = await portfolio_service.get_transactions(db=db, broker=request.broker)

        logger.info(
            f"Transactions accessed by user: {current_user.user_id}, days: {request.days}, broker: {request.broker}"
        )
        return transactions
...
async def get_performance_metrics(
    ...
    try:
        metrics = await portfolio_service.get_performance_metrics(
            db, current_user.user_id, request.timeframe, request.benchmark
        )

        logger.info(
            f"Performance metrics accessed by user: {current_user.user_id}, timeframe: {request.timeframe}"
        )
...
async def get_risk_metrics(
    ...
    try:
        metrics = await portfolio_service.get_risk_metrics(
            db, current_user.user_id, request.timeframe
        )

        logger.info(
            f"Risk metrics accessed by user: {current_user.user_id}, timeframe: {request.timeframe}"
        )
        return metrics.model_dump()
...
async def get_rebalancing_suggestions(
    ...
    try:
        suggestions = await portfolio_service.get_rebalancing_suggestions(
            db, current_user.user_id, request.target_allocation, request.drift_threshold
        )
...
async def run_stress_test(
    ...
    try:
        results = await portfolio_service.run_stress_test(
            db, current_user.user_id, request.scenarios
        )
