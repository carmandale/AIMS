...
async def run_stress_test(
    ...
    try:
        ...
        logger.info(f"Stress test run by user: {current_user.user_id}")
        return results
...
async def get_position_risk_contributions(
    ...
    """Get risk contribution of each position for authenticated user"""
    
    # Check rate limit
    portfolio_rate_limiter.check_rate_limit(current_user.user_id)
    
    try:
        contributions = await portfolio_service.get_position_risk_contributions(
            db, 
            current_user.user_id
        )
        
        logger.info(f"Risk contributions accessed by user: {current_user.user_id}")
        return contributions
        
    except Exception as e:
        logger.error(f"Failed to get risk contributions for user {current_user.user_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_correlation_matrix(
    ...
    """Get correlation matrix between assets for authenticated user"""
    
    # Check rate limit
    portfolio_rate_limiter.check_rate_limit(current_user.user_id)
    
    try:
        matrix = await portfolio_service.get_correlation_matrix(db, current_user.user_id)
        
        logger.info(f"Correlation matrix accessed by user: {current_user.user_id}")
        return matrix
        
    except Exception as e:
        logger.error(f"Failed to get correlation matrix for user {current_user.user_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))