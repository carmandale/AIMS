...
async def register_snaptrade_user(
    ...
    try:
        ...
        existing_snaptrade_user = (
            db.query(SnapTradeUser).filter(SnapTradeUser.user_id == current_user.user_id).first()
        )
...
async def delete_snaptrade_user(
    ...
    try:
        ...
        return UserDeletionResponse(
            status="deleted", message="SnapTrade user and all associated data have been deleted"
        )
...
async def handle_connection_callback(
    ...
    try:
        ...
        logger.info(
            f"Callback processed for user {current_user.user_id}, found {len(accounts_data)} accounts"
        )
