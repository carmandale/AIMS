...
class TaskService:
    ...
    async def generate_task_instances(
        ...
        logger.info(f"Generated {len(created_instances)} task instances for {start_date} to {end_date}")
    ...
    async def update_task_status(
        ...
        logger.info(f"Task status updated: {task.name} (ID: {task_id}) from {old_status} to {status}")
        return task
    ...
    async def get_compliance_metrics(
        ...
        overdue_tasks = sum(1 for t in tasks if t.status in ["pending", "in_progress"] and t.due_date < datetime.now())
