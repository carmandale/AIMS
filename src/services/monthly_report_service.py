...
class MonthlyReportService:
    ...
    def _calculate_monthly_performance(
        ...
        monthly_return_percent = (monthly_return / start_value * 100) if start_value > 0 else Decimal("0")
        ...
        avg_daily_return = Decimal(str(statistics.mean(daily_returns))) if daily_returns else Decimal("0")
        
        # Get YTD performance
        ytd_start = date(report_month.year, 1, 1)
        ytd_snapshots = db.query(PerformanceSnapshot).filter(
            and_(
                PerformanceSnapshot.user_id == user_id,
        ...
        if ytd_snapshots and len(ytd_snapshots) > 1:
            ...
            ytd_return_percent = (ytd_return / ytd_start_value * 100) if ytd_start_value > 0 else Decimal("0")
