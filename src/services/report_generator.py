...
from src.data.models.portfolio import (
    ...
    AssetAllocation, Report, PortfolioSummary
)
...
class ReportGenerator:
    ...
    async def generate_report(
        ...
        try:
            ...
            file_path = await self._generate_file(
                content, report_type, file_format, db_report.id
            )
    ...
    async def get_report(self, db: Session, report_id: int) -> Optional[Report]:
        """Get report by ID"""
        db_report = db.query(db_models.Report).filter(
    ...
    async def get_user_reports(
        ...
        query = db.query(db_models.Report).filter(
            db_models.Report.user_id == user_id
        )
        
        if report_type:
            query = query.filter(db_models.Report.report_type == report_type)
        
        db_reports = query.order_by(
            db_models.Report.created_at.desc()
        ).limit(limit).all()
    ...
    async def delete_expired_reports(self, db: Session) -> int:
        ...
        now = datetime.utcnow()
        expired_reports = db.query(db_models.Report).filter(
            db_models.Report.expires_at < now
        ).all()
    ...
    def _calculate_dividend_income(self, transactions: List[Transaction]) -> Dict[str, Any]:
        ...
        return {
            ...
            "dividends_by_symbol": {
                symbol: float(amount) 
                for symbol, amount in dividends_by_symbol.items()
            }
        }
    ...
    async def _write_summary_csv(self, csvfile, content: Dict[str, Any]):
        ...
        writer.writerow(["Symbol", "Broker", "Quantity", "Cost Basis", "Current Price", "Market Value", "P&L"])
