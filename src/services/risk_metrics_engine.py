...
class RiskMetricsEngine:
    ...
    async def calculate_comprehensive_risk_metrics(
        ...
        beta, alpha, r_squared = await self._calculate_beta_alpha(
            daily_returns, timeframe
        )
    ...
    async def calculate_position_risk_contribution(
        ...
        for position in positions:
            ...
            risk_contributions[position.symbol] = {
                ...
                "specific_risk": weight * math.sqrt(max(0, position_volatility**2 - position_beta**2))
            }
    ...
    async def calculate_portfolio_stress_test(
        ...
        if not scenarios:
            scenarios = [
                ...
                {"name": "Interest Rate Spike", "market_change": -0.15, "volatility_multiplier": 1.3},
        ...
        for scenario in scenarios:
            ...
            for position in positions:
                ...
                position_multiplier = self._get_scenario_multiplier(
                    position, scenario_name
                )
                
                position_scenario_change = market_change * position_multiplier * volatility_multiplier
                position_new_value = position.market_value * (1 + Decimal(str(position_scenario_change)))
    ...
    async def calculate_concentration_risk(
        ...
        return {
            ...
            "concentration_score": self._calculate_concentration_score(herfindahl_index, len(positions))
        }
    ...
    async def _calculate_beta_alpha(
        ...
        covariance = sum(
            (p - mean_portfolio) * (m - mean_market) 
            for p, m in zip(daily_returns, market_returns)
        ) / len(daily_returns)
