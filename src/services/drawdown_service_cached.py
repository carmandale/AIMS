...
class CachedDrawdownService(DrawdownService):
    ...
    def calculate_drawdown_events_cached(
        ...
        # Load snapshots if not provided
        if snapshots is None:
            query = db.query(PerformanceSnapshot).filter(PerformanceSnapshot.user_id == user_id)
    ...
    def get_historical_analysis_cached(
        ...
        # Get analysis
        result = self.get_historical_analysis(snapshots, start_date, end_date, threshold_percent)
    ...
    def get_or_calculate_all(
        ...
        return {
            "current_drawdown": self.calculate_current_drawdown_cached(db, user_id, snapshots),
