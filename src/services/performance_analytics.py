...
class PerformanceAnalyticsService:
    ...
    def calculate_returns_metrics(
        ...
        if days_difference > 0 and starting_value > 0:
            daily_return = (float(ending_value) / float(starting_value)) ** (1 / days_difference) - 1
    ...
    def get_period_performance(
        ...
        if period == "daily":
            ...
            earliest = self.db.query(PerformanceSnapshot.snapshot_date).filter(
                PerformanceSnapshot.user_id == user_id
            ).order_by(PerformanceSnapshot.snapshot_date).first()
    ...
    def _get_snapshots_for_period(
        ...
        return self.db.query(PerformanceSnapshot).filter(
            and_(
                PerformanceSnapshot.user_id == user_id,
