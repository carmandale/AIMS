...
from src.data.models.portfolio import (
    Position, AssetAllocation, ConcentrationRisk, RebalancingAction
)
...
class AllocationAnalyzer:
    ...
    async def analyze_allocation(
        ...
        by_asset_class = self._calculate_asset_class_allocation(positions, cash_balance, total_value)
            ...
            if abs(drift) > drift_threshold:
                ...
                if drift > 0:  # Overweight - need to sell
                    ...
                    if relevant_positions:
                        largest_position = max(relevant_positions, key=lambda p: p.market_value or 0)
