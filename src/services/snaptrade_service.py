...
class SnapTradeService:
    ...
    async def generate_connection_url(self, user_id: str, user_secret: str, connection_type: str = "read") -> Optional[str]:
        """
            ...
            if response.body:
                for position in response.body:
                    ...
                    positions.append({
                        ...
                        "quantity": float(position.get("quantity", 0)) if position.get("quantity") else 0.0,
                        "average_purchase_price": float(position.get("average_purchase_price", 0)) if position.get("average_purchase_price") else 0.0,
                        "last_ask_price": float(position.get("last_ask_price", 0)) if position.get("last_ask_price") else 0.0,
                        "market_value": float(position.get("market_value", 0)) if position.get("market_value") else 0.0,
    ...
    async def get_account_balances(self, user_id: str, user_secret: str, account_id: str) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if response.body and "balances" in response.body:
                for balance in response.body["balances"]:
                    ...
                    currency = currency_info.get("code", "USD") if currency_info else "USD"
                    balances[f"cash_{currency}"] = float(balance.get("cash", 0)) if balance.get("cash") else 0.0
                    balances[f"buying_power_{currency}"] = float(balance.get("buying_power", 0)) if balance.get("buying_power") else 0.0
            ...
            if response.body:
                for transaction in response.body:
                    ...
                    transactions.append({
                        ...
                        "quantity": float(transaction.get("quantity", 0)) if transaction.get("quantity") else 0.0,
                        "price": float(transaction.get("price", 0)) if transaction.get("price") else 0.0,
                        "currency": currency_info.get("code", "USD") if currency_info else "USD",
