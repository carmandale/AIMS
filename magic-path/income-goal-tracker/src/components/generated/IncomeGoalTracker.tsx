...
export const IncomeGoalTracker: React.FC<IncomeGoalTrackerProps> = ({
  currentIncome = 8750,
  monthlyGoal = 12000,
  weeklyData = [{
    week: 1,
  ...
  return <motion.div initial={{
    ...
    ease: 'easeOut'
      ...
      <div className="flex justify-center mb-10">
        <MonthlyProgressRing currentIncome={currentIncome} monthlyGoal={monthlyGoal} progressPercentage={progressPercentage} dailyAverageNeeded={dailyAverageNeeded} />
      </div>

      {/* Weekly Breakdown */}
      <div className="space-y-4">
        <h2 className="text-lg font-semibold text-white mb-4">Weekly Breakdown</h2>
        {weeklyData.map((week, index) => <motion.div key={week.week} initial={{
        opacity: 0,
        x: -20
      }} animate={{
        opacity: 1,
        ...
        delay: index * 0.1
            ...
            <WeeklyProgressBar week={week.week} actual={week.actual} target={week.target} label={week.label} />
          </motion.div>)}
      </div>
