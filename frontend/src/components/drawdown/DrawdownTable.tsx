						...
						{sortedEvents.map((event, index) => {
							...
							return (
								...
								>
									...
									<td className="py-4 pr-4">
										<p className="text-sm font-medium text-white">
											{event.recovery_days > 0
												? formatDuration(event.recovery_days)
												: '—'}
										</p>
									</td>
									<td className="py-4 pr-4">
										<div className={`inline-flex items-center space-x-2 px-2 py-1 rounded-lg ${statusInfo.bg}`}>
									...
									<td className="py-4">
										<div className="text-sm">
											...
											<p className="text-slate-400">
												→ {formatCurrency(event.trough_value)}
											</p>
											{event.is_recovered && (
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
					...
					<div>
						<p className="text-2xl font-bold text-red-400">
							{formatPercent(
								Math.min(...events.map(e => e.max_drawdown_percent))
							)}
						</p>
					...
					<div>
						<p className="text-2xl font-bold text-yellow-400">
							{formatDuration(
								Math.round(
									events.reduce((sum, e) => sum + e.duration_days, 0) / events.length
								)
							)}
						</p>
					...
					<div>
						<p className="text-2xl font-bold text-emerald-400">
							{Math.round(
								(events.filter(e => e.is_recovered).length / events.length) * 100
							)}%
						</p>
