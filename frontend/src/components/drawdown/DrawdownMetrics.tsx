...
const MetricCard: React.FC<MetricCardProps> = ({
	...
	if (isLoading) {
		return (
			<div className={`bg-slate-900/50 backdrop-blur-sm rounded-xl border ${getStatusBorder()} p-6`}>
	...
	return (
		...
		>
			<div className="flex items-center justify-between mb-4">
				...
				{status !== 'normal' && (
					<AlertTriangle className={`w-5 h-5 ${status === 'warning' ? 'text-yellow-400' : 'text-red-400'}`} />
				)}
			</div>
				...
				>
					<div className="flex items-center justify-between">
						...
						<div className="text-right">
							...
							<p className="text-xs text-slate-400">
								{formatCurrency(benchmarkDrawdown.amount)}
							</p>
						</div>
					</div>
									...
									width: `${Math.min(
										...
										Math.max(
											...
											(Math.abs(benchmarkDrawdown.percentage) /
												Math.max(Math.abs(currentDrawdown.percentage), Math.abs(benchmarkDrawdown.percentage))) *
												100
										)
									)}%`,
								}}
					...
					<div className="text-center">
						...
						<p className="text-sm font-medium text-purple-400">
							{Math.abs(currentDrawdown.percentage) > Math.abs(statistics.max_drawdown_ever) * 0.8
								? 'High vs History'
								: Math.abs(currentDrawdown.percentage) > Math.abs(statistics.max_drawdown_ever) * 0.5
								? 'Moderate vs History'
								: 'Low vs History'}
						</p>
					</div>
