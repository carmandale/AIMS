...
describe('DrawdownDashboard', () => {
  const mockUseCurrentDrawdownMetrics =
    useDrawdownHooks.useCurrentDrawdownMetrics as jest.MockedFunction<
      typeof useDrawdownHooks.useCurrentDrawdownMetrics
    >;
  const mockUseHistoricalDrawdownData =
    useDrawdownHooks.useHistoricalDrawdownData as jest.MockedFunction<
  ...
  it('displays alert status when alerts are active', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('Portfolio drawdown (16.2%) exceeds warning threshold of 15%')
      ).toBeInTheDocument();
    });
  });
  ...
  it('handles empty state when no historical data is available', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('No significant drawdown events found in the selected period.')
      ).toBeInTheDocument();
    });
  });
