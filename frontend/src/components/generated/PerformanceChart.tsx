    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-slate-800/95 backdrop-blur-sm border border-slate-600 rounded-lg p-3 shadow-xl">
          ...
          <p className="text-white font-semibold">${data.value.toLocaleString()}</p>
          <p
            className={`text-sm font-medium ${data.return >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
          >
            {data.return >= 0 ? '+' : ''}
        ...
        <div className="flex items-center space-x-1 bg-slate-800/50 rounded-lg p-1">
          {timeframes.map(timeframe => (
            <button
              ...
              className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${selectedTimeframe === timeframe.value ? 'bg-blue-600 text-white' : 'text-slate-300 hover:text-white hover:bg-slate-700'}`}
            >
              {timeframe.label}
            </button>
          ))}
