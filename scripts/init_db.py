...
def verify_database_setup(db: Session) -> bool:
    ...
    try:
        ...
        expected_tables = {
            'users', 'snaptrade_users', 'brokerage_accounts', 'positions', 
            'balances', 'transactions', 'morning_brief', 'cached_data',
            'task_status', 'task_templates', 'task_instances', 'task_audit_log',
            'tax_lots', 'sync_logs', 'performance_snapshots', 'reports',
            'asset_allocations'
        }
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="Force recreate all tables (DELETES ALL DATA)")
