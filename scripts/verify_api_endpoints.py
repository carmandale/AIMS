...
def verify_performance_data():
    ...
    try:
        ...
        print(f"   Value range: ${snapshots[0].total_value:,.2f} to ${snapshots[-1].total_value:,.2f}")
...
def verify_drawdown_service():
    ...
    try:
        ...
        for i, event in enumerate(events[:3], 1):  # Show first 3 events
            print(f"   Event {i}: {event['max_drawdown_percent']:.2f}% over {event['drawdown_days']} days")
        ...
        alerts = drawdown_service.check_alert_thresholds(
            ...
            critical_threshold=Decimal("20.0")
        )
...
def test_api_endpoint_logic():
    ...
    try:
        ...
        drawdown_response = {
            ...
            "peak_date": current_drawdown["peak_date"].isoformat() if current_drawdown["peak_date"] else None,
            "current_value": str(current_drawdown["current_value"]),
            "current_date": current_drawdown["current_date"].isoformat() if current_drawdown["current_date"] else None,
...
def main():
    ...
    if passed == total:
        ...
        print("\n📋 Next steps:")
        print("   1. Start backend server: cd .. && uv run uvicorn src.api.main:app --reload --port 8002")
