...
def create_realistic_performance_data(
    ...
    while current_date <= end_date:
        ...
        current_drawdown = (high_water_mark - current_value) if high_water_mark > current_value else Decimal("0")
        current_drawdown_percent = (current_drawdown / high_water_mark * 100) if high_water_mark > 0 else Decimal("0")
        ...
        if len(snapshots) >= 7:
            ...
            weekly_pnl_percent = (weekly_pnl / week_ago_value * 100) if week_ago_value > 0 else Decimal("0")
        ...
        if len(snapshots) >= 30:
            ...
            monthly_pnl_percent = (monthly_pnl / month_ago_value * 100) if month_ago_value > 0 else Decimal("0")
        ...
        if snapshots:
            for prev_snapshot in snapshots:
                if prev_snapshot.current_drawdown_percent and prev_snapshot.current_drawdown_percent > max_drawdown:
...
def generate_comprehensive_test_data():
    ...
    try:
        ...
        account_ids = [
            acc.id for acc in db.query(BrokerageAccount).filter(
                BrokerageAccount.user_id == test_user.user_id
            ).all()
        ]
        ...
        db.query(BrokerageAccount).filter(
            BrokerageAccount.user_id == test_user.user_id
        ).delete()
        ...
        for snapshot in snapshots:
            if snapshot.current_drawdown_percent and snapshot.current_drawdown_percent > 5:  # 5% threshold
                if not in_drawdown:
                    drawdown_events += 1
                    in_drawdown = True
            else:
...
def verify_test_data():
    ...
    try:
        ...
        significant_drawdowns = [
            ...
            if s.current_drawdown_percent and s.current_drawdown_percent > 10
        ]
