...
class TestDrawdownPerformance:
    ...
    def test_calculate_current_drawdown_performance(self, service, generate_snapshots):
        ...
        for days, label in test_cases:
            ...
            assert (
                execution_time < 0.1
            ), f"Current drawdown calculation took too long for {label}: {execution_time:.4f}s"

        # Print performance results
        print("\nCurrent Drawdown Calculation Performance:")
        for result in results:
            print(
                f"  {result['label']:10s} ({result['snapshots_count']:4d} snapshots): {result['execution_time']*1000:.2f}ms"
            )
    ...
    def test_calculate_drawdown_events_performance(self, service, generate_snapshots):
        ...
        for days, label in test_cases:
            snapshots = generate_snapshots(
                days, volatility=0.03
            )  # Higher volatility for more events
            ...
            assert (
                execution_time < 0.5
            ), f"Drawdown events calculation took too long for {label}: {execution_time:.4f}s"

        # Print performance results
        print("\nDrawdown Events Calculation Performance:")
        for result in results:
            print(
                f"  {result['label']:10s} ({result['snapshots_count']:4d} snapshots, {result['events_count']:2d} events): {result['execution_time']*1000:.2f}ms"
            )
    ...
    def test_calculate_underwater_curve_performance(self, service, generate_snapshots):
        ...
        for days, label in test_cases:
            ...
            assert (
                execution_time < 0.2
            ), f"Underwater curve calculation took too long for {label}: {execution_time:.4f}s"

        # Print performance results
        print("\nUnderwater Curve Calculation Performance:")
        for result in results:
            print(
                f"  {result['label']:10s} ({result['points_count']:4d} points): {result['execution_time']*1000:.2f}ms"
            )
    ...
    def test_historical_analysis_performance(self, service, generate_snapshots):
        ...
        for days, label in test_cases:
            ...
            # Measure execution time
            start_time = time.time()
            analysis = service.get_historical_analysis(snapshots, threshold_percent=Decimal("3.0"))
            execution_time = time.time() - start_time
            ...
            assert (
                execution_time < 1.0
            ), f"Historical analysis took too long for {label}: {execution_time:.4f}s"

        # Print performance results
        print("\nHistorical Analysis Performance:")
        for result in results:
            print(
                f"  {result['label']:10s} ({result['total_events']:2d} events): {result['execution_time']*1000:.2f}ms"
            )
