...
class TestSnapTradeToPerformanceFlow:
    ...
    def test_snaptrade_to_performance_snapshot_flow(self, mock_balances, mock_positions, mock_accounts):
    ...
    def test_real_time_performance_updates_simulation(self):
        ...
        for i, movement in enumerate(market_movements):
            ...
            intraday_snapshot = PerformanceSnapshot(
                ...
                total_return=Decimal(str(daily_change * 100)),
                daily_return=Decimal(str(movement["change"] * 100)),
                num_positions=10,
                created_at=datetime.utcnow()
            )
            ...
            print(f"✅ Real-time update {movement['time']}: ${new_value:,.2f} ({cumulative_change:+.1%})")
    ...
    def test_benchmark_integration_flow(self, mock_benchmark_data):
        ...
        for i in range(31):
            ...
            snapshot = PerformanceSnapshot(
                ...
                total_return=Decimal(str(portfolio_growth * 100)),
                daily_return=Decimal(str(0.02 * ((-1) ** i))),  # Daily volatility
                num_positions=10,
                created_at=datetime.utcnow()
            )
            self.db.add(snapshot)
        ...
        mock_benchmark_data.return_value = {
            "returns": {
                (base_date + timedelta(days=i)).isoformat(): 0.06 * (i / 365)  # 6% annual growth for benchmark
                for i in range(31)
            },
        ...
        print(f"   - Outperformance: {portfolio_metrics['total_return'] - benchmark_metrics['total_return']:.2%}")
        ...
        for scenario in test_scenarios:
            ...
            for i, value in enumerate(scenario["daily_values"]):
                ...
                if i == 0:
                    ...
                    total_return = (value - scenario["daily_values"][0]) / scenario["daily_values"][0]
                    daily_return = (value - scenario["daily_values"][i-1]) / scenario["daily_values"][i-1]
    ...
    def test_historical_data_consistency(self):
        ...
        for i in range(91):  # 0 to 90 days
            ...
            snapshot = PerformanceSnapshot(
                ...
                cash_value=Decimal("10000.00"),
                positions_value=Decimal(str(portfolio_value - 10000)),
                total_return=Decimal(str((growth + volatility) * 100)),
                daily_return=Decimal(str(volatility * 100)),
                num_positions=10,
                created_at=datetime.utcnow()
            )
            self.db.add(snapshot)
